...
export const getHistory = async (type) => {
    try {
        if (type === 'keywords') {
            ...
            console.log("Invalid history type: ${type}. Please use 'keywords' or 'selections'.");
        }
    } catch (error) {
...
export const getSearchHistory = async () => {
    try {
        ...
        let choices = keywords.map(item => ({
            name: `${item.keyword} (${item.type}) - ${new Date(item.timestamp).toLocaleString()}`,
            value: item
        }));
        
        // sort by timestamp
        choices.sort((a, b) => new Date(b.value.timestamp) - new Date(a.value.timestamp));
...
export const getSelectionHistory = async () => {
    try {
        ...
        let choices = selections.map(item => ({
            name: `${item.name} (${item.type}) - ${new Date(item.timestamp).toLocaleString()}`,
            value: item
        }));
        ...
        const itemDetails = await api.getByID(accessToken, selectedItem.type, selectedItem.id);
